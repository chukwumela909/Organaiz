...
export default function PushNotifications() {
  ...
  const sendTestCall = useCallback(async () => {
    try {
      const res = await fetch("/api/push/call", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ caller: "Organaiz" }),
      });
      const data = await res.json();
      console.log("[Push] Test call result:", data);
    } catch (err) {
    ...
    <>
      ...
      {showButton && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
          ...
          {/* Test call button — only when subscribed */}
          {isSubscribed && (
            <button
              onClick={sendTestCall}
              className="flex h-10 items-center gap-2 rounded-full bg-blue-600 px-4 text-xs font-medium text-white shadow-lg transition-all hover:bg-blue-500 active:scale-95"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-4 w-4">
